...
<x:tbody title="项目结算查询" iconCls="icon-dot">
  <x:grid id="first" hql="from ProjectAccount" idfield="id" onDblClickRow="windowOpen('项目结算信息','/c/jsp/account/sendAccountBook'+rec.id);">
      ...
      <table class="userInput_table" cellspacing="0" cellpadding="0"
        ...
        <tr>
          ...
          <td class="td_context"><select name="select">
            <option>--请选择--</option>
            <option>施工单位已申请</option>
            <option>建设单位同意</option>
            <option>制作审计报告</option>
            <option>已经提交审计</option>
            <option>已完成结算</option>
	...
	<x:colsWrap>
      ...
      <x:col width="200" title="项目名称" field="projectName" sortable="true" frozen="true"/>
      ...
      <x:col width="100" title="施工单位" field="bbb" formatter="'施工单位'+rec.id"/>
      <x:col width="100" title="日期" field="aaaa" formatter="'2010-11-30'"/>
      <x:col width="100" title="状态" field="status"/>
    </x:colsWrap>
	<x:grm iconCls="icon-ok" handler="windowOpen('项目结算信息','/c/jsp/account/sendAccountBook'+rowData.id);" text="查看"/>
  </x:grid>
</x:tbody>
